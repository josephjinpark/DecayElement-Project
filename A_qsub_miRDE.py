...
def include(sChrID_mir, sChrID_read, list_nArmPos, list_nMirPos):
    ...
    if sChrID_mir.lower() != sChrID_read.lower():
        ...
        if (nStartPos_mi <= nStartPos_read) and (nEndPos_mi >= nEndPos_read): bFlag = 0
        else: bFlag = 1
    #if END: sChrID
...
def qsub_Segmentation (sTargetSeedType, sGenome, sQueue, bTestRun, sSegmentWindow):
    ...
    sOutDir     = '%s/out/%s/%s/%sbps/01_segments'     % (sBASE_DIR, sGenome, sTargetSeedType, sSegmentWindow)
    sLogDir     = '%s/log/%s/%s/%sbps/01_segments/%s'  % (sBASE_DIR, sGenome, sTargetSeedType, sSegmentWindow, sTIME_STAMP)
    ...
    for sChrID in cGenome.sChrIDList:
        for sStrand in ['+','-']:
            ...
            sScript = '%s/temp/B_core_miRDE_%s.py segmentation %s %s %s %s %s %s' %\
                      (sBASE_DIR, sTIME_STAMP, sOutDir, sGenome, sTargetSeedType, sChrID, sStrand, sSegmentWindow)
            ...
            if bTestRun == True:
                ...
                os.system('echo "%s" | qsub -cwd -j y -o %s -q %s -N Jinman.mirDE.Segments.%s.%s%s.%s.%sbps' %
                          (sScript, sLogDir, sQueue, sGenome, sChrID, sStrand, sTargetSeedType, sSegmentWindow))
        #loop END: sStrand
    #loop END: sChrID
...
def qsub_Filter_seed_conserve (sTargetSeedType, sGenome, sQueue, bTestRun, sSegmentWindow):
    sInDir       = '%s/out/%s/%s/%sbps/01_segments'            % (sBASE_DIR, sGenome, sTargetSeedType, sSegmentWindow)
    sLogDir      = '%s/log/%s/%s/%sbps/02_filter_conserve/%s'  % (sBASE_DIR, sGenome, sTargetSeedType, sSegmentWindow, sTIME_STAMP)

    cGenome      = cFasta(dict_sGENOME_DIR[sGenome])
    list_sChrIDs = ['%s%s' % (sChrID, sStrand) for sChrID in cGenome.sChrIDList for sStrand in ['+', '-']]

    for sChrID in list_sChrIDs:
        sOutDir  = '%s/out/%s/%s/%sbps/02_filter_conserve/%s' % (sBASE_DIR, sGenome, sTargetSeedType, sSegmentWindow, sChrID)
        os.makedirs(sOutDir, exist_ok=True)

        sScript  = '%s/temp/B_core_miRDE_%s.py filter_by_seed_conservation %s %s %s %s %s %s' % \
                    (sBASE_DIR, sTIME_STAMP, sInDir, sOutDir, sGenome, sChrID, sTargetSeedType, sSegmentWindow)

        if bTestRun == True:
            print(sScript)
        else:
            os.makedirs(sLogDir, exist_ok=True)
            os.system('echo "%s" | qsub -cwd -j y -o %s -q %s -hold_jid Jinman.mirDE.Segments* -N Jinman.mirDE.FilterSeedCons.%s.%s.%s.%sbps' %
                      (sScript, sLogDir, sQueue, sGenome, sChrID, sTargetSeedType, sSegmentWindow))
    #loop END: sChrID
#def END: qsub_Filter_seed_conserve


def qsub_RNAfold_and_localAU (sTargetSeedType, sGenome, sQueue, bTestRun, sSegmentWindow):
    sInDir       = '%s/out/%s/%s/%sbps/02_filter_conserve'     % (sBASE_DIR, sGenome, sTargetSeedType, sSegmentWindow)
    sLogDir      = '%s/log/%s/%s/%sbps/03_rnafold_localAU/%s'  % (sBASE_DIR, sGenome, sTargetSeedType, sSegmentWindow, sTIME_STAMP)
    ...
    for sChrID in list_sChrIDs:
        ...
        sOutDir  = '%s/out/%s/%s/%sbps/03_rnafold_localAU/%s' % (sBASE_DIR, sGenome, sTargetSeedType, sSegmentWindow, sChrID)
        os.makedirs(sOutDir, exist_ok=True)

        sScript  = '%s/temp/B_core_miRDE_%s.py RNAfold_and_localAU %s %s %s %s %s %s' % \
                    (sBASE_DIR, sTIME_STAMP, sInDir, sOutDir, sGenome, sChrID, sTargetSeedType, sSegmentWindow)
        ...
        if bTestRun == True:
            ...
            os.makedirs(sLogDir, exist_ok=True)
            os.system('echo "%s" | qsub -cwd -j y -o %s -q %s -hold_jid Jinman.mirDE.FilterSeedCons* -N Jinman.mirDE.RNAfold.%s.%s.%s.%sbps' %
                      (sScript, sLogDir, sQueue, sGenome, sChrID, sTargetSeedType, sSegmentWindow))
    #loop END: sChrID
#def END: qsub_RNAfold_and_localAU


def qsub_Filter_dot_bracket (sTargetSeedType, sGenome, sQueue, bTestRun, sSegmentWindow):
    sInDir       = '%s/out/%s/%s/%sbps/03_rnafold_localAU'     % (sBASE_DIR, sGenome, sTargetSeedType, sSegmentWindow)
    sLogDir      = '%s/log/%s/%s/%sbps/04_dotbr_filtered/%s'   % (sBASE_DIR, sGenome, sTargetSeedType, sSegmentWindow, sTIME_STAMP)
    ...
    for sChrID in list_sChrIDs:
        ...
        sOutDir     = '%s/out/%s/%s/%sbps/04_dotbr_filtered/%s'       % (sBASE_DIR, sGenome, sTargetSeedType, sSegmentWindow, sChrID)
        os.makedirs(sOutDir, exist_ok=True)

        sScript     = '%s/temp/B_core_miRDE_%s.py filter_dot_bracket %s %s %s %s %s' % \
                        (sBASE_DIR, sTIME_STAMP, sInDir, sOutDir, sGenome, sChrID, sSegmentWindow)

        if bTestRun == True:
            print(sScript)
        else:
            os.makedirs(sLogDir, exist_ok=True)
            os.system('echo "%s" | qsub -cwd -j y -o %s -q %s -hold_jid Jinman.mirDE.RNAfold* -N Jinman.mirDE.Filter.%s.%s.%s.%sbps' %
                      (sScript, sLogDir, sQueue, sGenome, sChrID, sTargetSeedType, sSegmentWindow))
    #loop END: sChrID
#def END: qsub_Filter_dot_bracket


def qsub_Apply_additional_constraints (sTargetSeedType, sGenome, sQueue, bTestRun, sSegmentWindow):
    sInDir       = '%s/out/%s/%s/%sbps/04_dotbr_filtered'            % (sBASE_DIR, sGenome, sTargetSeedType, sSegmentWindow)
    sLogDir      = '%s/log/%s/%s/%sbps/05_additional_constraints/%s' % (sBASE_DIR, sGenome, sTargetSeedType, sSegmentWindow, sTIME_STAMP)
    ...
    for sChrID in list_sChrIDs:
        ...
        sOutDir     = '%s/out/%s/%s/%sbps/05_additional_constraints/%s'% (sBASE_DIR, sGenome, sTargetSeedType, sSegmentWindow, sChrID)
        os.makedirs(sOutDir, exist_ok=True)

        sScript     = '%s/temp/B_core_miRDE_%s.py apply_additional_constraints %s %s %s %s %s' % \
                        (sBASE_DIR, sTIME_STAMP, sInDir, sOutDir, sGenome, sChrID, sSegmentWindow)
        ...
        if bTestRun == True:
            ...
            os.makedirs(sLogDir, exist_ok=True)
            os.system('echo "%s" | qsub -cwd -j y -o %s -q %s -hold_jid Jinman.mirDE.Filter*  -N Jinman.mirDE.AddConstraints.%s.%s.%s.%sbps' %
                      (sScript, sLogDir, sQueue, sGenome, sChrID, sTargetSeedType, sSegmentWindow))
    #loop END: sChrID
#def END: qsub_Apply_additional_constraints


def qsub_Sort_and_evaluate_segments (sTargetSeedType, sGenome, sQueue, bTestRun, sSegmentWindow):
    sInDir  = '%s/out/%s/%s/%sbps/05_additional_constraints'   % (sBASE_DIR, sGenome, sTargetSeedType, sSegmentWindow)
    sOutDir = '%s/out/%s/%s/%sbps/06_sort_and_evaluate'        % (sBASE_DIR, sGenome, sTargetSeedType, sSegmentWindow)
    sLogDir = '%s/log/%s/%s/%sbps/06_sort_and_evaluate/%s'     % (sBASE_DIR, sGenome, sTargetSeedType, sSegmentWindow, sTIME_STAMP)
    ...
    for sMirName in list_sMirName:
        ...
        sScript = '%s/temp/B_core_miRDE_%s.py sort_and_evaluate_segments %s %s %s %s %s' % \
                  (sBASE_DIR, sTIME_STAMP, sInDir, sOutDir, sGenome, sMirName, sSegmentWindow)

        if bTestRun == True:
            print(sScript)
        else:
            os.makedirs(sLogDir, exist_ok=True)
            os.system('echo "%s" | qsub -cwd -j y -o %s -q %s -hold_jid Jinman.mirDE.AddConstraints* -N Jinman.mirDE.SortEval.%s.%s.%s.%sbps' %
                      (sScript, sLogDir, sQueue, sGenome, sMirName, sTargetSeedType, sSegmentWindow))
    #loop END: sMirName
#def END: qsub_Sort_and_evaluate_segments


def qsub_Full_compile_list (sTargetSeedType, sGenome, sQueue, bTestRun, sSegmentWindow, sSortBy):
    sInDir  = '%s/out/%s/%s/%sbps/06_sort_and_evaluate'    % (sBASE_DIR, sGenome, sTargetSeedType, sSegmentWindow)
    sOutDir = '%s/out/%s/%s/%sbps/07_full_compile_list'    % (sBASE_DIR, sGenome, sTargetSeedType, sSegmentWindow)
    sLogDir = '%s/log/%s/%s/%sbps/07_full_compile_list/%s' % (sBASE_DIR, sGenome, sTargetSeedType, sSegmentWindow, sTIME_STAMP)

    os.makedirs(sOutDir, exist_ok=True)

    sScript = '%s/temp/B_core_miRDE_%s.py full_compile_list %s %s %s %s %s' % \
              (sBASE_DIR, sTIME_STAMP, sInDir, sOutDir, sSegmentWindow, sTargetSeedType, sSortBy)

    if bTestRun == True:
        print(sScript)
    else:
        os.makedirs(sLogDir, exist_ok=True)
        os.system('echo "%s" | qsub -cwd -j y -o %s -q %s -hold_jid Jinman.mirDE.SortEval* -N Jinman.mirDE.FullList.%s.%s.%sbps.%s' %
                  (sScript, sLogDir, sQueue, sGenome, sTargetSeedType, sSegmentWindow, sSortBy))
    #if END: bTestRun
#def END: qsub_Full_compile_list


def qsub_extended_ss (sTargetSeedType, sGenome, sQueue, bTestRun, sSegmentWindow, sSortBy):
    sInDir  = '%s/out/%s/%s/75bps/07_full_compile_list'     % (sBASE_DIR, sGenome, sTargetSeedType)
    sOutDir = '%s/out/%s/%s/75bps/08_extended_ss'           % (sBASE_DIR, sGenome, sTargetSeedType)
    sLogDir = '%s/log/%s/%s/75bps/08_extended_ss/%s'        % (sBASE_DIR, sGenome, sTargetSeedType, sTIME_STAMP)

    os.makedirs(sOutDir, exist_ok=True)

    sScript = '%s/temp/B_core_miRDE_%s.py extended_ss %s %s %s %s %s' % \
              (sBASE_DIR, sTIME_STAMP, sInDir, sOutDir, sSegmentWindow, sTargetSeedType, sSortBy)
    ...
    if bTestRun == True:
        ...
        os.makedirs(sLogDir, exist_ok=True)
        os.system('echo "%s" | qsub -cwd -j y -o %s -q %s -hold_jid Jinman.mirDE.FullList* -N Jinman.mirDE.Extended.%s.%s.%sbps.%s' %
                  (sScript, sLogDir, sQueue, sGenome, sTargetSeedType, sSegmentWindow, sSortBy))
    #if END: bTestRun
#def END: qsub_Full_compile_list
...
def qsub_compile_targetmir_rpkm_list (sGenome, sAnalysis, sStudy,  sQueue, bTestRun):
    ...
    sScript = '%s/temp/B_core_miRDE_%s.py get_target_mir_rpkm_list %s %s %s %s %s' % \
              (sBASE_DIR, sTIME_STAMP, sGenome, sAnalysis, sStudy,  sInDir, sOutDir)
...
def main():
    ...
    #Part 2: Filter Segments by SeedSite Conservation Sore
    #qsub_Filter_seed_conserve          (sTargetSeedType, sGenome, sQueue, bTestRun, sSegmentWindow)

    #Part 3: Calculate Basic Stats
    #local_Basic_stats                  (sTargetSeedType, sSegmentWindow)

    #Part 4: Generate Dot-Bracket Notation and Calculate Min. Free Energy and LocalAU Sore
    qsub_RNAfold_and_localAU           (sTargetSeedType, sGenome, sQueue, bTestRun, sSegmentWindow)

    #Part 5: Filter/Screen by Dot-Bracket Notation
    #qsub_Filter_dot_bracket             (sTargetSeedType, sGenome, sQueue, bTestRun, sSegmentWindow)

    #Part 6: Apply Additional Constraints
    #qsub_Apply_additional_constraints   (sTargetSeedType, sGenome, sQueue, bTestRun, sSegmentWindow)

    #Part 7: Compile Filtered Data,  Sort and Output Secondary Structure
    #qsub_Sort_and_evaluate_segments     (sTargetSeedType, sGenome, sQueue, bTestRun, sSegmentWindow)

    #Part 8: Full Compiled List, Sort and Output Secondary Structure
    #qsub_Full_compile_list             (sTargetSeedType, sGenome, sQueue, bTestRun, sSegmentWindow, sSortBy)

    #Part 9: Extend Segment Sequence for Secondary Structure
    qsub_extended_ss                   (sTargetSeedType, sGenome, sQueue, bTestRun, sSegmentWindow, sSortBy)

    #Part 10: Compare 75, 100, 125 bps list
    ...
    #Part 11: Expression Profile from small RNA and mRNA Sequencing Data (Cell-Lines)
    #qsub_get_maturemir_RPM             (sGenome, sQueue, bTestRun)

    #qsub_compile_targetmir_rpm_list    (sAnalysis, sGenome, sQueue, bTestRun)

    #qsub_compile_targetmir_rpkm_list   (sGenome, sAnalysis, sStudy, sQueue, bTestRun)
